...
(function ($) {
  ...
  $.ThreeSixty = function (el, options) {
    ...
    base.imageLoaded = function () {
      ...
      $(AppCongif.progress + " span").text(
        Math.floor((AppCongif.loadedImages / AppCongif.totalFrames) * 100) +
          "%",
      );
      ...
      if (AppCongif.disableWrap) {
        ...
        if (n - realEndFrame > 0) {
          ...
          if (n - realEndFrame < realEndFrame + (AppCongif.totalFrames - n)) {
            ...
            newEndFrame =
              AppCongif.endFrame - (realEndFrame + (AppCongif.totalFrames - n));
          }
        } else {
    ...
    base.trackPointer = function (event) {
      if (AppCongif.ready && AppCongif.dragging) {
        ...
        if (
          AppCongif.monitorStartTime <
          new Date().getTime() - AppCongif.monitorInt
        ) {
          AppCongif.pointerDistance =
            AppCongif.pointerEndPosX - AppCongif.pointerStartPosX;
          if (AppCongif.pointerDistance > 0) {
            AppCongif.endFrame =
              AppCongif.currentFrame +
              Math.ceil(
                (AppCongif.totalFrames - 1) *
                  AppCongif.speedMultiplier *
                  (AppCongif.pointerDistance / base.$el.width()),
              );
          } else {
            AppCongif.endFrame =
              AppCongif.currentFrame +
          ...
          if (AppCongif.disableWrap) {
            AppCongif.endFrame = Math.min(
              AppCongif.totalFrames - (AppCongif.zeroBased ? 1 : 0),
              AppCongif.endFrame,
            );
            AppCongif.endFrame = Math.max(
              AppCongif.zeroBased ? 0 : 1,
              AppCongif.endFrame,
            );
          }
          base.refresh();
    ...
    base.refresh = function () {
      if (AppCongif.ticker === 0) {
        AppCongif.ticker = setInterval(
          base.render,
          Math.round(1000 / AppCongif.framerate),
        );
      }
    };
    /**
    ...
    base.render = function () {
      ...
      if (AppCongif.currentFrame !== AppCongif.endFrame) {
        frameEasing =
          AppCongif.endFrame < AppCongif.currentFrame
            ? Math.floor((AppCongif.endFrame - AppCongif.currentFrame) * 0.1)
            : Math.ceil((AppCongif.endFrame - AppCongif.currentFrame) * 0.1);
        base.hidePreviousFrame();
        AppCongif.currentFrame += frameEasing;
        base.showCurrentFrame();
        base.$el.trigger("frameIndexChanged", [
          base.getNormalizedCurrentFrame(),
          AppCongif.totalFrames,
        ]);
      } else {
